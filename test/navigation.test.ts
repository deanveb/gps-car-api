...
describe('shortestPath module', () => {
  test('correct shortest path from M to P', () => {
    const coords = navigation.generateGraph();
    expect((navigation.findShortestPath(coords, [6, 1], [6, 6]) as navigation.coordinate[]).map(pos => pos.position)).toEqual([[6, 1], [4, 1], [4, 3], [6, 3], [6, 4], [6, 6]]);
  });
});

describe('getDirection module', () => {
  test('given vectors when starting out at M', () => {
    expect(navigation.getDirection(new navigation.vector(undefined), new navigation.vector([2, 0]))).toBe("thang");
  });
  test('give correct direction when starting out at M and the car is at J then go to K', () => {
    expect(navigation.getDirection(new navigation.vector([2, 0]), new navigation.vector([0, 2]))).toBe("phai");
  });
  test('give correct direction when starting out at M and the car is at J then go to R', () => {
    expect(navigation.getDirection(new navigation.vector([2, 0]), new navigation.vector([0, -1]))).toBe("trai");
  });
})