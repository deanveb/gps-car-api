...
export default async function handler(req: any, res: any) {
  ...
  const startPosition = [parseInt(req.query.start0), parseInt(req.query["start1"])];
  const endPosition = [parseInt(req.query.end0), parseInt(req.query.end1)];

  const result = findShortestPath(coordinates, startPosition, endPosition);
  // Check for error
  if (typeof result == "string") {
    return res.status(400).json({ error: result, start: startPosition, end: endPosition, queries: req.query});
  }
